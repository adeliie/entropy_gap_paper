...
data_rel = np.load(DATA_FILE_SIGN_BIG, allow_pickle=True)
data_sgd = np.load(DATA_FILE_SIGN)
data_real = np.load(DATA_FILE_REAL_SIGN, allow_pickle=True)
...
(line_th1,) = ax1.plot(
    Ts_p1, 1.0 / Ts_p1, linestyle="--", color="crimson", linewidth=2.0
)
leg_dims = ax1.legend(
    h_p1_dims,
    l_p1_dims,
