...
init_ce_err = (-torch.sum(pi_j * Pi_mat * torch.log(F.softmax(W0_ce, dim=0) + 1e-30)).item()) - entropy
...
opt_ce_adam_list = [torch.optim.Adam([p], lr=eta, betas=(0.0, 0.999), eps=1e-15) for p, eta in zip(W_ce_adam_list, eta_grid_ce_adam)]
opt_q_adam_list = [torch.optim.Adam([p], lr=eta, betas=(0.0, 0.999), eps=1e-15) for p, eta in zip(W_q_adam_list, eta_grid_q_adam)]
    ...
    for k in range(T_steps + 1):
        if k in set(range(0, T_steps + 1, eval_interval)):
            ...
            for i, W in enumerate(W_ce_gd_list):
                P = F.softmax(W, dim=0)
                hist_all['ce_gd'][i, eval_idx] = (-torch.sum(pi_j * Pi_mat * torch.log(P + 1e-30)).item() - entropy) / init_ce_err
            for i, W in enumerate(W_q_gd_list):
                hist_all['q_gd'][i, eval_idx] = (0.5 * torch.sum(pi_j * (W - Pi_mat) ** 2).item()) / init_q_err
            for i, W in enumerate(W_ce_sd_list):
                P = F.softmax(W, dim=0)
                hist_all['ce_sd'][i, eval_idx] = (-torch.sum(pi_j * Pi_mat * torch.log(P + 1e-30)).item() - entropy) / init_ce_err
            for i, W in enumerate(W_q_sd_list):
                hist_all['q_sd'][i, eval_idx] = (0.5 * torch.sum(pi_j * (W - Pi_mat) ** 2).item()) / init_q_err
            for i, p in enumerate(W_ce_adam_list):
                P = F.softmax(p, dim=0)
                hist_all['ce_adam'][i, eval_idx] = (-torch.sum(pi_j * Pi_mat * torch.log(P + 1e-30)).item() - entropy) / init_ce_err
            for i, p in enumerate(W_q_adam_list):
                hist_all['q_adam'][i, eval_idx] = (0.5 * torch.sum(pi_j * (p - Pi_mat) ** 2).item()) / init_q_err
            eval_idx += 1
