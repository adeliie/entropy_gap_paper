...
from package.plot_functions import nice_logspace
...
grid = nice_logspace(-10, 10, density=3, base=10)
eta_grid_ce_gd = grid
eta_grid_q_gd = grid
eta_grid_ce_sd = grid
eta_grid_q_sd = grid
eta_grid_ce_adam = grid
eta_grid_q_adam = grid
num_etas = len(grid)
...
opt_ce_adam_list = [
    torch.optim.Adam([p], lr=eta, betas=(0.0, 0.999))
...
for key in hist_all:
    if "sd" in key or "adam" in key:
        ...
        best_eta_idx = np.nanargmin(hist_all[key][:, -1])
