...
for d in ds:
    ...
    if f'd_{d}_{eta_name}_err' in results_dict:
        print(f"    -> eta = {eta_name} déjà calculé. Passage au suivant.")
    else:
        print(f"    -> Calcul pour eta = {eta_name} ")
            
        saved_t, err_matrix = f.gradient_real_data(vocab_size=d, T=T, eta=0, N_checkpoints=100)
        ...
        print(f"       Terminé pour {eta_name}. Données sauvegardées.")

   
    print(f" -> Fin pour d={d} en {time.time() - t0:.2f}s.\n")
