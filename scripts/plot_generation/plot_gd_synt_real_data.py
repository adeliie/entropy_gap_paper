...
data_gd = np.load("data/data_gd_matrix.npz", allow_pickle=True)
ds_gd = data_gd['ds']
data_gf = np.load("data/data_gf_eta=logd^2.npz", allow_pickle=True)
ds_gf = data_gf['ds']
...
if hasattr(pf, 'theory_global'):
    ...
    line_th1, = ax1.plot(tau_theory, R_th_glob, '--', color='red', linewidth=2.5, zorder=10)
    line_th2, = ax2.plot(tau_theory, R_th_glob, '--', color='red', linewidth=2.5, zorder=10) 
...
ax1.legend(handles_ax1, labels_ax1, loc='upper right', borderaxespad=0., frameon=False, handletextpad=0.8)
...
for d in ds_real:
    ...
    if tau_key in data_real and err_key in data_real:
        ...
        line_real, = ax2.plot(tau_real, err_global_real, color=col, linestyle='-', linewidth=2.5)
